...
void castSpell(const std::string& spell, Hero& hero, Enemy& enemy, WorldLevel currentLevel) {
    if (spell == "IGNIS") {
    ...
    else if (spell == "VAMPIRUM" and (currentLevel == WorldLevel::BURNED_JUNGLE or currentLevel == WorldLevel::CRATER)) {
    ...
    else if (spell == "GLACIA" and currentLevel != WorldLevel::WASTELAND) {
    ...
    else if (spell == "VITA" and currentLevel != WorldLevel::SNOW_RUINS and currentLevel != WorldLevel::WASTELAND) {
        std::cout << "Свет наполняет тебя. Боль отступает.\n";
        hero.hp += 15;
        if (hero.hp > hero.maxHp) hero.hp = hero.maxHp;
        hero.clearNegativeEffects();
    }
    else if (spell == "UMBRA" and currentLevel == WorldLevel::CRATER) {
...
void combat(Hero& player, Enemy& enemy, sf::Font& font, WorldLevel currentLevel) {
    ...
    while (player.hp > 0 && enemy.hp > 0) {
        ...
        if (actionChoice == 1) {
            ...
            castSpell(result.spell, player, enemy, currentLevel);
        }
...
void finalChoice() {
    ...
    case 1:
        ...
        typeWriter("Голос в голове : ЧТО ТЫ ДЕЛАЕШЬ?\n");
...
void exploreLevel() {
    ...
    typeWriter("\n🧙‍Ты приходишь в себя, лёжа на потрескавшейся земле. Ветер тащит за собой пепел. "
        ...
        "\n\tIGNIS – сильное заклинание, наносит 5 едениц урона и поджигает врага. "
            ...
            if (step < maxSteps) {
                if (currentLevel == WorldLevel::WASTELAND) {
                    if (step % 2 == 0) {
                        Zombie zombie;
                        Enemy& enemy = zombie;
                        combat(player, enemy, font, currentLevel);
                    }
                    else {
                        Skeleton skeleton;
                ...
                if (currentLevel == WorldLevel::WASTELAND) {
                    ...
                    typeWriter("\nГолос в голове : Чем дальше ты заходишь, тем суровее условия. "
                        ...
                        "\n\tGLACIA Обрушивает ледяной удар, нанося 5 едениц урона и с шансом 50 % замораживает врага на 1 ход."
