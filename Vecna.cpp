...
#include <chrono>
#include <windows.h> // Для изменения кодировки на Windows
#include <map>
#include <thread>
#include <iomanip>
#include <sstream>
...
SpellResult spellInputWindow(sf::Font& font) {
    ...
    return { "", 0.0f }; // если окно закрылось без нажатия Cast
}
...
public:
    ...
    void attack(Hero& hero) override {
        ...
        if (rand() % 4 == 0) { // 25% шанс двойной атаки
...
void castSpell(const std::string& spell, Hero& hero, Enemy& enemy) {
    if (spell == "IGNIS") {
        ...
        hero.hp += 3;
        if (hero.hp > hero.maxHp) hero.hp = hero.maxHp;
        hero.applyEffect({ VAMP_BUFF, 3 }); // условный бафф на урон (в будущем можно использовать)
        enemy.onAttacked(hero);
    }
...
void restCamp(Hero& player) {
    ...
    std::cout << "2. Поспать\n";
    std::cout << "3. Открыть инвентарь (пока не реализовано)\n";
    std::cout << "4. Поговорить с голосом (пока не реализовано)\n";
...
void finalChoice() {
    ...
    case 1:
        ...
        std::cout << "ЧТО ТЫ ДЕЛАЕШЬ?\n";
        std::cout << "Ты пытаешься сконцентрировать всю свою магическую силу лишь на одном...\n";
        std::cout << "Голос в голове : СТОЙ! ОСТАНОВИСЬ! НЕ ДЕЛАЙ ЭТОГО!\n";
        std::cout << "Ты представляешь мгновения прошлого, которые тебе довелось вспомнить... Мама. Академия. Покой.\n";
        std::cout << "***Яркая вспышка***\n";
        break;
    case 2:
        std::cout << "\nТы медленно берёшь кольцо. Оно пульсирует.\n";
        std::cout << "Как только ты надеваешь его, всё замирает.\n";
        std::cout << "Ты чувствуешь силу. Силу, которую не может постичь ни один смертный разум.\n";
        std::cout << "Но с этой силой уходит нечто ещё. Тепло. Память. Человечность.\n";
        std::cout << "Ты больше не Векна. Ты — Архилич.\n";
        break;
    case 3:
        ...
        std::cout << "Голос в голове : СТОЙ! ПОЖАЛУЙСТА! НЕ УХОДИ... НЕ ОСТАВЛЯЙ МЕНЯ...\n";
        std::cout << "Оно остаётся там, в центре кратера. Ожидает. Но ты — свободен.\n";
        std::cout << "Мир пуст, суров и жесток. Но в нём ты — человек.\n";
        std::cout << "А может, это и есть настоящая магия.\n";
        break;
    default:
        std::cout << "\nТы не делаешь выбор.\n";
        std::cout << "И всё остаётся так, как есть. Голос затихает. Кольцо тускнеет.\n";
        std::cout << "Мир замирает, ожидая, когда ты вновь будешь готов выбрать.\n";
    }
}
...
void exploreLevel() {
    ...
    while (true) {
        ...
        if (action == 1) {
            ...
            std::cout << "\nТы находишься в локации: " << getLevelName(currentLevel) << "\n";
            ...
            if (step < maxSteps) {
                ...
                if (currentLevel == WorldLevel::WASTELAND) {
                    ...
                    typeWriter("\nГолос в голове : Чем дальше ты заходишь, тем суровее условия. "
                        ...
                        "\n'name': слушай, тут такое дело, когда я использую новое заклинание, я вижу какие-то видения..."
            ...
            // std::cout << "\nТы идёшь вперёд... Враг приближается!\n";
            // combat(player, enemy, font);
            // std::this_thread::sleep_for(std::chrono::seconds(1));
            // std::cout << "\nТы побеждаешь врага.\n";
            // currentLevel = static_cast<WorldLevel>(static_cast<int>(currentLevel) + 1);
            // if (currentLevel == WorldLevel::COMPLETED) {
                //     std::cout << "Ты прошёл все уровни. Истина близка...\n";
                //     break;
                // }
        }
        else if (action == 2) {
            ...
            std::cout << "Неверный выбор. Попробуй ещё раз.\n";
        }
    }
}
// Главная функция
